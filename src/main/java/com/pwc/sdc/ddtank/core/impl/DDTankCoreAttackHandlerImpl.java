...
import com.pwc.sdc.ddtank.core.*;
import com.pwc.sdc.ddtank.core.*;
import com.pwc.sdc.ddtank.exception.DDTankAngleResolveException;
...
import com.pwc.sdc.ddtank.util.FileUtils;
import com.pwc.sdc.ddtank.core.DDTankCoreAttackHandler;
...
/*                else if (properties.getIsHandleAttack()) {
            ...
            // 进入副本后延迟为外面的 1/10
            ThreadUtils.delay(properties.getDelay() / 10, true);
            ++failTimes;
        }
        // 下次再调用时仍然能继续调用
        exit = false;
    ...
    private void attackAuto() {
        ...
        if(System.currentTimeMillis() - start < 200) {
            // 按下空格后延迟一小段时间
            ThreadUtils.delay(200 - (System.currentTimeMillis() - start), true);
        }
        ddtankOperate.attack(strength);
    }
